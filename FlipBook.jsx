...
//       title: "750ml Water Bottles",
//       images: [
//         "/img1.png", // Corrected path from user's "/img1.png"
//         "/img2.png",
//         "/img3.png",
//         "/img4.png",
//         "/img5.png",
...
const FlipBook = () => {
  ...
  // Enhanced Product Gallery Component with Amazon-style cards
 // Optimized Product Gallery Component for FlipBook
const ProductGallery = ({ productId, closeGallery, colors }) => {
  const product = productData[productId]
  if (!product) return null
  ...
  return (
    ...
    >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4 md:gap-6 max-w-7xl mx-auto">
          {product.images.map((image, index) => (
            <motion.div
              ...
              className="bg-gradient-to-br from-white to-green-50 rounded-xl overflow-hidden shadow-md transition-all duration-300 ease-in-out cursor-pointer border border-gray-100 hover:border-green-400 hover:shadow-xl group"
              onClick={() => setSelectedProduct({ id: productId, imageIndex: index })}
              whileHover={{ y: -4, scale: 1.02 }} // Reduced animation for better performance
              whileTap={{ scale: 0.98 }}
            >
              ...
              <div className="relative aspect-[4/3] overflow-hidden bg-green-50">
                <WatermarkedImage
                  ...
                  objectFit="contain"
                  className="transition-transform duration-300 group-hover:scale-105"
                  isPreview={true} // Enable preview mode for faster loading
                  priority={index < 4} // Prioritize first 4 images
                  watermarkText="VedicJal"
                />
                
                {/* Hover overlay with better performance */}
                <div className="absolute inset-0 bg-gradient-to-t from-black/0 via-transparent to-transparent group-hover:from-black/10 transition-all duration-300 flex items-end justify-center pb-4">
                  <div className="opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-white/90 backdrop-blur-sm px-3 py-1 rounded-full text-sm font-medium text-gray-800 transform translate-y-2 group-hover:translate-y-0">
                    Click to zoom
                  </div>
                </div>
              </div>
              ...
              <div className="p-4">
                <h3 className="text-gray-800 font-semibold text-base mb-2 line-clamp-2">
                  {product.title} - Design {index + 1}
                </h3>
                
                <div className="flex items-center justify-between mb-2">
                  <span className="text-green-600 font-medium text-sm">Premium Quality</span>
                  <div className="flex items-center gap-1 text-yellow-500">
                    {[...Array(5)].map((_, i) => (
                ...
                <p className="text-gray-600 text-sm mb-3">Customizable water bottle with eco-friendly materials</p>
                
                {/* Action Button */}
                <motion.button
                  className="w-full bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 text-white py-2 px-4 rounded-lg font-medium transition-all duration-200 shadow-sm hover:shadow-md text-sm"
                  whileHover={{ scale: 1.02 }}
                  whileTap={{ scale: 0.98 }}
                >
                  View Full Size
      ...
      <div className="p-4 bg-gradient-to-r from-green-900/30 to-emerald-900/30 backdrop-blur-md border-t border-green-500/20">
        <div className="text-center text-white/80 text-sm">
          <p>
            Showing {product.images.length} designs • All images are watermarked • 
            <span className="text-green-300 font-medium ml-1">Click any image for full view</span>
          </p>
        </div>
      </div>
    </motion.div>
...
// Enhanced Zoomed Image Viewer with better performance
// Updated ZoomedImageViewer component in your FlipBook.tsx
      ...
      <div className="relative w-[90vw] h-[90vh] overflow-hidden flex items-center justify-center">
        ...
        >
          <WatermarkedImage
            ...
            fill
            objectFit="contain"
            priority={true} // 🔥 This enables instant loading for zoom view
            watermarkText="VedicJal"
            onLoad={() => setIsImageLoaded(true)} // 🔥 Callback when loaded
          />
        </motion.div>
      </div>
      
      {/* Show loading only if image hasn't loaded yet */}
      {!isImageLoaded && (
